...
object KafkaProtocol {
  ...
  case class OrderCreatedEvent(
                                ...
                                title: String, price: Double, quantity: Int,
                                eventTimestamp: LocalDate, timeout: Long) extends KafkaEvent

  import org.apache.avro.specific.SpecificRecord

  def modelToAVROConverter(event: KafkaEvent): SpecificRecord =
    event match {
      case OrderCreatedEvent(id, title, price, quantity, eventTimestamp, timeout) =>
